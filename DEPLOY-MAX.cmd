...
IF NOT EXIST "%SOURCE%%PACKAGE%" ECHO  %PACKAGE% не найден! & TIMEOUT /T 3 >NUL & EXIT /B 1
...
IF NOT EXIST "%TMPDIR%\MAX" ECHO  Ошибка извлечения файлов! & TIMEOUT /T 3 >NUL & EXIT /B 1
ROBOCOPY "%TMPDIR%\MAX" "%TARGET%" /MOVE /E /NFL /NDL /NJH /NJS /ETA
RMDIR /S /Q "%TMPDIR%"

POWERSHELL.EXE -NoProfile -Command "$sh=(New-Object -COM WScript.Shell).CreateShortcut('%LNKPATH%');$sh.TargetPath='%TARGET%\max.exe';$sh.Save()"
